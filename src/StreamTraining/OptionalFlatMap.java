 ...
 * которые хранятся его в учетной записи (у пользователя одна учетная запись, а в ней есть список счетов).
 * Если вам нужно как-то обработать счета пользователя,
 * то можно сделать преобразование из одного Optional в другой Optional методом flatMap, который есть в Optional.
...
public class OptionalFlatMap {
    public static Optional<Integer> flatMap(List<String> strings) {
        return strings.stream()
                .filter(a -> a.endsWith(".java")) // проверяет заканчивается ли строка на это
                .findFirst()// Метод findFirst() возвращает первый элемент по порядку из Stream-a, в виде обертки Optional
                .flatMap(s -> Optional.of(s.length())); // преобразование из одного Optional в другой Optional методом flatMap
    }
}